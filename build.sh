...
SETUP_BUILD() {
    ...
    if [ $SINGLEBUILD = "yes" ]; then
	  #  make -C "$RDIR" O=$BDIR CROSS_COMPILE=$CROSS_COMPILE $COMMON_DEFCONFIG $BOARD_DEFCONFIG $DEVICE_DEFCONFIG $DEBUG_DEFCONFIG \
        ...
        make -C "$RDIR" O=$BDIR  ARCH=arm64 LD=aarch64-linux-gnu-ld.bfd CC=clang CLANG_TRIPLE=aarch64-linux-gnu- CROSS_COMPILE=aarch64-linux-gnu- CROSS_COMPILE_ARM32=arm-linux-gnueabi- $COMMON_DEFCONFIG $DEBUG_DEFCONFIG $BOARD_DEFCONFIG  $DEVICE_DEFCONFIG
...
BUILD_KERNEL() {
	    ...
	    TIMESTAMP1=$(date +%s)
    if [ $SINGLEBUILD = "yes" ]; then
        while ! make -C "$RDIR" O=$BDIR -j"$THREADS" ARCH=arm64 CC=clang CLANG_TRIPLE=aarch64-linux-gnu- CROSS_COMPILE=aarch64-linux-gnu- CROSS_COMPILE_ARM32=arm-linux-gnueabi- LLVM=1 LLVM_IAS=1; do
