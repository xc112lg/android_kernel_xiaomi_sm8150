...
u8 fod_param[8] = { 0x44, 0x64, 0x44, 0x78, 0x3A, 0x67, 0x37, 0x67 };
...
static void rx1619_set_reverse_parameter(struct rx1619_chg *chip, u8 mode,
					 u8 iin)
{
...
if (gpio_get_value(chip->power_good_gpio) ==
				    0) {
					break;
				}
...
RETRY:
	...
	if (g_fw_data_lenth == (ret * 4)) {
		status = rx1619_update_fw_confirm_data(
			chip, area, 0x66); //0x66(check success)
		if (!status) {
			dev_err(chip->dev,
...
union power_supply_propval val = {
	...
	case 11: //PD-27W(20W)
		if (chip->op_mode != LN8282_OPMODE_SWITCHING) {
			dev_info(chip->dev,
"[20W]not switch mode, don't rise voltage \n");
			break;
		}
		/*
chip->batt_psy = power_supply_get_by_name("battery");
...
POWER_SUPPLY_PROP_WIRELESS_CP_EN, &val);
	...
	case 14:
		if (chip->op_mode != LN8282_OPMODE_SWITCHING) {
			dev_info(chip->dev,
"[30W]not switch mode, don't rise voltage \n");
			break;
		}
		/*
chip->batt_psy = power_supply_get_by_name("battery");
...
power_supply_get_property(chip->batt_psy, POWER_SUPPLY_PROP_CAPACITY,
				  &val);
	soc = val.intval;

power_supply_get_property(chip->batt_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW,
				  &val);
	vol_now = val.intval;

power_supply_get_property(chip->batt_psy, POWER_SUPPLY_PROP_CURRENT_NOW,
				  &val);
	cur_now = val.intval;
...
if (soc == FULL_SOC && batt_sts == POWER_SUPPLY_STATUS_FULL)
	...
	if (g_USB_TYPE >= 9) {
		if (chip->op_mode != LN8282_OPMODE_SWITCHING) {
			dev_info(chip->dev,
"not switch mode, don't adjust voltage \n");
			goto out;
		}
...
{
	struct rx1619_chg *chip =
		container_of(work, struct rx1619_chg, rx_first_boot.work);
...
{
	struct rx1619_chg *chip =
		container_of(work, struct rx1619_chg, chg_monitor_work.work);
...
{
	struct rx1619_chg *chip =
		container_of(work, struct rx1619_chg, fw_download_work.work);
		...
		if (is_valid_fw && (g_fw_rx_id >= FW_VERSION)) {
			...
			if (!rx1619_onekey_download_firmware(chip))
				...
				dev_info(chip->dev,
					 ...
					 __func__);
				boot_fw_version =
					(~fw_data_boot[sizeof(fw_data_boot) -
						       1]) &
					0xff;
				tx_fw_version =
...
{
	struct rx1619_chg *chip =
		container_of(work, struct rx1619_chg, pan_tx_work.work);
...
{
	struct rx1619_chg *chip =
		container_of(work, struct rx1619_chg, voice_tx_work.work);
...
POWER_SUPPLY_PROP_TEMP, &val);
	...
	if (batt_temp >= 402) {
		...
		if (chip->fcc_votable) {
			effective_fcc = 1000;
			vote(chip->fcc_votable, VOICE_LIMIT_FCC_1A_VOTER, true,
			     effective_fcc * 1000);
		}
	} else if (batt_temp < 400) {
		if (chip->fcc_votable)
			vote(chip->fcc_votable, VOICE_LIMIT_FCC_1A_VOTER, false,
			     0);
	}
	...
	if (batt_temp >= 375) {
		...
		if (chip->fcc_votable) {
			effective_fcc = 2600;
			vote(chip->fcc_votable, VOICE_LIMIT_FCC_VOTER, true,
			     effective_fcc * 1000);
		}
	} else if (batt_temp < 367) {
		if (chip->fcc_votable)
			vote(chip->fcc_votable, VOICE_LIMIT_FCC_VOTER, false,
			     0);
	}
...
{
	struct rx1619_chg *chip =
		container_of(work, struct rx1619_chg, chg_detect_work.work);
union power_supply_propval val = {
		0,
	};
...
union power_supply_propval wk_val = {
	...
	g_chip = chip;
power_supply_get_property(chip->usb_psy, POWER_SUPPLY_PROP_ONLINE,
				  &val);
...
static void rx1619_wireless_int_work(struct work_struct *work)
{
	...
	int rc;
	int cnt;
union power_supply_propval cp_val = {
		0,
	};
	int fc_flag = 0;
	int vol = 0;
...
chip->wireless_psy = power_supply_get_by_name("wireless");
	...
	if (tx_gpio) {
		if (rx1619_is_tx_mode(chip)) {
			rx1619_write(chip, AP_REV_DATA_OK,
				     REG_AP_RX_COMM); //receive ok
			...
			case PING:
				...
				if (tx_status == REVERSE_CHG_TX_SWITCH_DONE)
					dev_err(chip->dev,
						"reverse restart because of otg insertion\n");
				break;
			case TRANSFER:
...
POWER_SUPPLY_PROP_DIV_2_MODE, &cp_val);
				...
				dev_info(chip->dev,
					 "loop ln8282 set switch and get: %d\n",
					 chip->op_mode);
			}
			msleep(50);
...
if (chip->epp && chip->epp_max_power == 10) { //EPP
	...
	case 0x04: //id ok
		...
		rx1619_write(chip, PRIVATE_ID_CMD,
			     REG_RX_SENT_CMD); //0x86 authen req
		rx1619_write(chip, AP_SENT_DATA_OK, REG_AP_RX_COMM);
...
"vendor:0x%x, module:0x%x, hw:0x%x and power:0x%x\n",
			...
			if (g_uuid_data[3] == 0x01 && g_uuid_data[1] == 0x2 &&
			    g_uuid_data[2] == 0x8 && g_uuid_data[0] == 0x6) {
				chip->is_car_tx = 1;
		...
		rx1619_write(chip, PRIVATE_USB_TYPE_CMD,
			     REG_RX_SENT_CMD); //0x87 usb type req
		rx1619_write(chip, AP_SENT_DATA_OK, REG_AP_RX_COMM);
	...
	case 0x0f: //error cmd
		...
		if (!chip->epp &&
			...
			if (err_cmd == ID_CMD && (retry_id < 3)) {
				...
				retry++;
				dev_info(chip->dev,
					 "[rx1619] dev auth failed retry %d\n",
					 retry);
				break;
			} else {
				retry = 0;
			...
			dev_info(
				chip->dev,
				"[rx1619]auth failed tx charger type set 0x%x\n",
				g_USB_TYPE);
			alarm_cancel(&chip->cmd_timeout_alarm);
			if (chip->epp) {
				chip->target_vol = ADAPTER_EPP_QC3_VOL;
if (chip->epp_max_power < 10000) {
uA = ((chip->epp_max_power - 1000) /
					      11) *
					     1000;
					if (chip->op_mode ==
...
power_supply_changed(chip->wireless_psy);
	...
	case 0x1f: //for product test
		...
		rx1619_write(chip, PRIVATE_PRODUCT_TEST_CMD,
			     REG_RX_SENT_CMD); //sent test cmd
		rx1619_write(chip, AP_SENT_DATA_OK, REG_AP_RX_COMM);
		break;
...
power_supply_changed(chip->wireless_psy);
		dev_info(chip->dev,
"reverse_charge, set ln8282 powerpath and opmode\n");
		msleep(100);
		if (enable) {
			if (chip->wireless_psy) {
				cp_val.intval = 3;
...
POWER_SUPPLY_PROP_WIRELESS_WAKELOCK,
			...
			if (chip->hw_country == CountryGlobal ||
			    chip->is_otg_insert) {
				rx1619_set_reverse_parameter(
					chip, REVERSE_COUNTRY_INTERNATIONAL,
					REVERSE_CURRENT_500MA);
			} else {
				rx1619_set_reverse_parameter(
					chip, REVERSE_COUNTRY_DOMESTIC,
					REVERSE_CURRENT_1000MA);
			}
			/* set reverse fod */
					...
					const char *buf, size_t count)
{
	...
	for (i = 0; i < 8; i++) {
		fod_param[i] =
			(char)simple_strtoul(buf, NULL, 16); //fod_param[0]
		buf += 3;
	}
					 ...
					 char *buf)
{
	...
	if (!ret) {
		...
		boot_fw_version = (~fw_data_boot[sizeof(fw_data_boot) - 1]) &
				  0xff;
		tx_fw_version = (~fw_data_tx[sizeof(fw_data_tx) - 1]) & 0xff;
		rx_fw_version = (~fw_data_rx[sizeof(fw_data_rx) - 1]) & 0xff;
		boot_project_id_h = (~fw_data_boot[sizeof(fw_data_boot) - 3]) &
				    0xff;
		boot_project_id_l = (~fw_data_boot[sizeof(fw_data_boot) - 2]) &
				    0xff;
...
static ssize_t txon_show(struct device *dev, struct device_attribute *attr,
			 char *buf)
{
	int mode;
...
static ssize_t txon_store(struct device *dev, struct device_attribute *attr,
			  const char *buf, size_t count)
{
	int ret, enable;
...
chip->wip_psy = devm_power_supply_register(chip->dev, &chip->wip_psy_d,
	...
	if (chip->client->irq) {
		ret = devm_request_threaded_irq(
			&chip->client->dev, chip->client->irq, NULL,
...
enable_irq_wake(chip->power_good_irq);
	...
	if (alarmtimer_get_rtcdev()) {
		alarm_init(&chip->cmd_timeout_alarm, ALARM_BOOTTIME,
			   cmd_timeout_alarm_cb);
	} else {
