...
unsigned int rx1618_get_rx_vbuck(struct rx1618_chg *chip)
{
	...
	dev_err(chip->dev,
		"[rx1618] [%s] Vbuck: data=0x%x, vbuck=%d mV ,data1 = 0x%x, data2= 0x%x \n",
...
unsigned int rx1618_get_rx_ibuck(struct rx1618_chg *chip)
{
	...
	ibuck = (rx1618_get_rx_vrect(chip) * rx1618_get_rx_irect(chip) * 95) /
		(rx1618_get_rx_vbuck(chip) * 100);
...
int rx1618_set_vbuck(struct rx1618_chg *chip, int volt)
{
	...
	if ((volt < 5166) &&
	    (volt > 9609)) //V bus_set =5166000uV+21224uV*[Vbus_set(0:7)] DEC
	{
		data = 0;
	} else {
	...
	rx1618_write(chip, PRIVATE_VBUCK_SET_CMD,
		     REG_RX_SENT_CMD); //sent set vbuck cmd
...
bool rx1618_check_firmware(struct rx1618_chg *chip)
{
	...
	for (i = 0; i < sizeof(fw_data); i++) {
		if (read_buffer[i] != ((~fw_data[i]) & 0xff)) {
			dev_err(chip->dev,
				"[rx1618] [%s] fw download Fail! read_buffer[i]=0x%x, ~fw_data[i]=0x%x, i=%d, sizeof(fw_data)=%ld\n",
				__func__, read_buffer[i], (~fw_data[i]) & 0xff,
				i, sizeof(fw_data));
...
bool rx1618_download_firmware(struct rx1618_chg *chip)
{
	...
	dev_err(chip->dev,
		"[rx1618] [%s] fw download Success! Firmware total data: %d bytes \n",
		__func__, i);
	...
	if (check_status) {
		dev_err(chip->dev,
			"[rx1618] [%s] RX1618 download firmware and check firmware Success! \n",
			__func__);
		return true;
	} else {
		dev_err(chip->dev,
			"[rx1618] [%s] RX1618 download firmware Fail, Please try again !!! \n",
			__func__);
		return false;
	}
}
...
static void rx1618_wireless_work(struct work_struct *work)
{
	struct rx1618_chg *chip =
		container_of(work, struct rx1618_chg, wireless_work.work);

	mutex_lock(&chip->wireless_chg_lock);
	schedule_delayed_work(&chip->wireless_work,
			      msecs_to_jiffies(RX1618_DELAY_MS));
...
chip->wireless_psy = power_supply_get_by_name("wireless");
	...
	dev_err(chip->dev,
		"[rx1618] [%s] privite_cmd,data=0x%x,0x%x,0x%x,0x%x,0x%x\n",
	...
	case 0x09: //ID ok
		if (g_id_done_flag == 0) {
			rx1618_write(chip, AP_REV_DATA_OK,
				     REG_AP_RX_COMM); //receive ok
			msleep(10);

			rx1618_write(chip, PRIVATE_ID_CMD,
				     REG_RX_SENT_CMD); //sent sar
			rx1618_write(chip, AP_SENT_DATA_OK, REG_AP_RX_COMM);
			g_id_done_flag = 1;
	...
	case 0x0A: //SAR ok
		if (g_sar_done_flag == 0) {
			...
			rx1618_write(chip, PRIVATE_USB_TYPE_CMD,
				     REG_RX_SENT_CMD); //sent usb type
			rx1618_write(chip, AP_SENT_DATA_OK, REG_AP_RX_COMM);
			g_sar_done_flag = 1;
	...
	case 0x12: //fast charge ok
		if (g_fast_chg_flag == 0) {
			...
			rx1618_write(chip, PRIVATE_TX_HW_ID_CMD,
				     REG_RX_SENT_CMD); //sent tx hw id req
			rx1618_write(chip, AP_SENT_DATA_OK, REG_AP_RX_COMM);
	...
	case 0x0B: //USB type
		if (g_usb_type_flag == 0) {
			...
			rx1618_write(chip, PRIVATE_FAST_CHG_CMD,
				     REG_RX_SENT_CMD); //sent fast charge
			rx1618_write(chip, AP_SENT_DATA_OK, REG_AP_RX_COMM);
			g_usb_type_flag = 1;
			...
			case 0: //other charger
				if ((g_id_done_flag == 0) &&
				    (g_epp_or_bpp == BPP_MODE)) //bpp and no id
				{
					rx1618_set_pmi_icl(chip,
							   DC_OTHER_CURRENT);
					dev_err(chip->dev,
						"[rx1618] [%s] bpp and no id---800mA \n",
						__func__);
				} else //ID ok
				{
					rx1618_set_pmi_icl(chip,
...
POWER_SUPPLY_PROP_WIRELESS_CP_EN,
	...
	case 0x0C: //Light screen
		if (g_light_screen_flag == 0) {
			...
			dev_err(chip->dev,
				"[rx1618] [%s] Light screen,fw_id,hw_id_h,hw_id_l,g_epp_or_bpp=0x%x, 0x%x, 0x%x, 0x%x\n",
				__func__, g_fw_id, g_hw_id_h, g_hw_id_l,
				g_epp_or_bpp);
		}
		break;
	...
	case 0x4C: //tx hw id
		...
		dev_err(chip->dev,
			"[rx1618] [%s] tx hw id,data1-3=0x%x, 0x%x, 0x%x, 0x%x\n",
	...
	case 0x0D: //TX test request
		...
		dev_err(chip->dev,
			"[rx1618] [%s] product Test mode, header=0x%x, header_length=0x%x\n",
		...
		dev_err(chip->dev,
			"[rx1618] [%s] tx_cmd,data=0x%x, 0x%x, 0x%x, 0x%x\n",
			__func__, tx_cmd, data1, data2, data3);

		if (tx_cmd == 0x12) //irect
		{
			rx1618_write(chip, 0x38,
			...
			dev_err(chip->dev,
				"[rx1618] [%s] product test--0x12--irect=%d \n",
				__func__, irect);
		...
		} else if (tx_cmd == 0x25) //hw id
		{
			...
			dev_err(chip->dev,
				"[rx1618] [%s] product test--0x25--g_hw_id=0x%x, 0x%x \n",
				__func__, g_hw_id_h, g_hw_id_l);
		} else {
			dev_err(chip->dev,
				"[rx1618] [%s] product test--other cmd \n",
				__func__);
			break;
		}

		dev_err(chip->dev,
			"[rx1618] [%s] header,tx_cmd,data=0x%x, 0x%x, 0x%x, 0x%x, 0x%x \n",
			__func__, header, tx_cmd, data1, data2, data3);

		rx1618_write(chip, PRIVATE_PRODUCT_TEST_CMD,
...
static int rx1618_parse_dt(struct rx1618_chg *chip)
{
	...
	if (gpio_is_valid(chip->irq_gpio)) {
		...
		if (ret) {
			dev_err(chip->dev,
				"[rx1618] [%s] irq gpio request failed, ret = %d\n",
				__func__, ret);
			return -EPROBE_DEFER;
		}
		...
		if (ret) {
			dev_err(chip->dev,
				"[rx1618] [%s] set direction failed, ret = %d\n",
				__func__, ret);
			goto fail_irq_gpio;
		}
				     ...
				     const char *buf, size_t count)
{
	...
	dev_err(g_chip->dev,
		"[rx1618] [%s] g_offset_l=0x%x, g_offset_h=0x%x \n", __func__,
		g_offset_l, g_offset_h);
					  ...
					  const char *buf, size_t count)
{
	...
	if (enable) {
		...
		if (!ret) {
			dev_err(g_chip->dev,
				"[rx1618] [%s] Firmware Update failed! Please try again! \n",
				__func__);
		}
		dev_err(g_chip->dev,
			"[rx1618] [%s] Firmware Update Success!!! \n",
			__func__);
	}
					 ...
					 char *buf)
{
	...
	if (ret) {
		return sprintf(buf,
			       "rx1618_download_firmware check Success!!!\n");
	} else {
...
static DEVICE_ATTR(chip_vbuck_calibration, S_IWUSR, NULL,
		   chip_vbuck_calibration_store);
static DEVICE_ATTR(chip_firmware_update, S_IWUSR | S_IRUGO,
		   chip_firmware_update_show, chip_firmware_update_store);
...
static DEVICE_ATTR(chip_enable, S_IWUSR | S_IRUGO, chip_enable_show,
		   chip_enable_store);
static DEVICE_ATTR(chip_vbuck, S_IWUSR | S_IRUGO, chip_vbuck_show,
		   chip_vbuck_store);
static DEVICE_ATTR(chip_ibuck, S_IRUGO, chip_ibuck_show, NULL);
static DEVICE_ATTR(chip_ap_req, S_IWUSR | S_IRUGO, chip_ap_req_show,
		   chip_ap_req_store);
...
static int rx1618_probe(struct i2c_client *client,
