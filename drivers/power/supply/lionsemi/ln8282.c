...
{
	regmap_update_bits(info->regmap, LN8282_REG_DEVICE_CTRL_1, 0x01,
			   (enable & 0x01));
}
...
{
	regmap_update_bits(info->regmap, LN8282_REG_DEVICE_CTRL_0, 0x01,
			   (enable & 0x01));
}
...
*/
static inline void ln8282_auto_recovery(struct ln8282_info *info,
					unsigned int enable)
{
	regmap_update_bits(info->regmap, LN8282_REG_DEVICE_CTRL_1, 0x40,
			   (enable << 6));
}
					  ...
					  unsigned int track_cfg)
{
	...
	regmap_update_bits(info->regmap, LN8282_REG_TRACK_CTRL, 0x10,
			   (track_cfg << 4));
...
{
	regmap_update_bits(info->regmap, LN8282_REG_TRACK_CTRL, 0x04,
			   (disable << 2));
}
					 ...
					 unsigned int alt_en)
{
	...
	regmap_update_bits(info->regmap, LN8282_REG_TRIM_4, 0x04,
			   (alt_en << 2));
...
*   This routine should be called when the RX IC or PMIC detects a new power source
	...
	regmap_update_bits(info->regmap, LN8282_REG_HV_SC_CTRL_1, 0x10,
			   (1 << 4));
	...
	regmap_update_bits(info->regmap, LN8282_REG_TRIM_7, LN8282_MASK_SWAP_EN,
			   0 /*<<7*/);
	regmap_write(info->regmap, LN8282_REG_LION_CTRL, 0x00);
#endif
...
static void ln8282_otp_write(struct ln8282_info *info, unsigned int otp_addr,
					 ...
					 const char *buf, size_t count)
{
	...
	if (write_cmd) {
		if (sscanf(buf, "write,%x,%x", &ln8282_regcmd_addr,
			   &ln8282_regcmd_data) != 2)
			goto parse_error;
...
static DEVICE_ATTR(opmode, 0644, ln8282_sysfs_show_opmode,
		   ln8282_sysfs_store_opmode);
static DEVICE_ATTR(regcmd, 0644, ln8282_sysfs_show_regcmd,
		   ln8282_sysfs_store_regcmd);
static struct attribute *ln8282_info_attr[] = {
	&dev_attr_dump.attr,	      &dev_attr_hw_init.attr,
#ifdef LN8282_DEBUG_OTP_CTRL
...
static int ln8282_gpio_init(struct ln8282_info *info)
{
	...
	ret = pinctrl_select_state(info->ln_pinctrl, info->ln_gpio_active);
	if (ret < 0) {
...
static int get_board_version(void)
{
	...
	char *match = (char *)strnstr(
		saved_command_line,
		"androidboot.hwlevel=", strlen(saved_command_line));
	if (match) {
...
struct power_supply_config ln_cfg = {};
	...
	if (sysfs_create_group(&info->client->dev.kobj, &ln8282_attr_group) <
	    0) {
...
static const struct i2c_device_id ln8282_id[] = { { "ln8282", 0 }, {} };
