...
#define ln_err(fmt, ...)                                                  \
	do {                                                              \
		if (info->dev_role == LN_PRIMARY)                         \
			...
			printk(KERN_ERR "ln8000@sec: %s: " fmt, __func__, \
			       ##__VA_ARGS__);                            \
	} while (0);
...
#define ln_info(fmt, ...)                                                  \
	do {                                                               \
		if (info->dev_role == LN_PRIMARY)                          \
			...
			printk(KERN_INFO "ln8000@sec: %s: " fmt, __func__, \
			       ##__VA_ARGS__);                             \
	} while (0);
...
#define ln_dbg(fmt, ...)                                                    \
	do {                                                                \
		if (info->dev_role == LN_PRIMARY)                           \
			...
			printk(KERN_DEBUG "ln8000@sec: %s: " fmt, __func__, \
			       ##__VA_ARGS__);                              \
	} while (0);
...
static int ln8000_create_debugfs_entries(struct ln8000_info *info)
{
	...
	info->debug_root = debugfs_create_dir(
		(LN8000_IS_PRIMARY(info) ? "ln8000" : "ln8000-secondary"),
		NULL);
	if (!info->debug_root) {
...
static const struct of_device_id ln8000_dt_match[] = {
	{ .compatible = "lionsemi,ln8000", .data = (void *)LN_PRIMARY },
...
static const struct i2c_device_id ln8000_id[] = { { "ln8000", LN_PRIMARY },
						  { "ln8000-secondary",
						    LN_SECONDARY },
						  {} };
