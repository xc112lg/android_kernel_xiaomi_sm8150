...
struct halo7221_func {
	...
	int (*read_buf)(struct halo7221_dev *di, u16 reg, u8 *buf, u32 size);
	int (*write_buf)(struct halo7221_dev *di, u16 reg, u8 *buf, u32 size);
	int (*write_masked)(struct halo7221_dev *di, u16 reg, u8 mask, u8 val);
};
...
static int halo7221_hw_init(struct halo7221_dev *chip)
{
	...
	msleep(200);
	rc = chip->bus.write_masked(chip, 0x03, GENMASK(1, 0),
				    0x01); // set 11v to enable cp mode
	msleep(50);
	rc = chip->bus.write_masked(chip, 0x01, GENMASK(7, 3),
				    0x1A); // Iin_Limit 1500mA
...
static ssize_t chip_init_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	int ret = 0;
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
...
static int halo7221_gpio_init(struct halo7221_dev *chip)
{
	...
	ret = pinctrl_select_state(chip->halo_pinctrl, chip->halo_gpio_active);
	if (ret < 0) {
...
static int halo7221_probe(struct i2c_client *client,
