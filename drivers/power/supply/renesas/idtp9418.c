	...
	if (!di->fw_update) {
		...
		for (i = 0; i < 3; i++) {
			...
			if (mode & BIT(0)) {
				...
				dev_err(di->dev,
					"idt set reverse charge failed, retry: %d\n",
					i);
			}
		}
...
power_supply_set_property(
			di->wireless_psy,
POWER_SUPPLY_PROP_REVERSE_PEN_CHG_STATE, &val);
power_supply_changed(di->wireless_psy);
...
{
	struct idtp9220_device_info *di = container_of(
		work, struct idtp9220_device_info, hall3_irq_work.work);
	if (di->hall3_online)
		idtp9220_set_reverse_enable(di, true);
...
{
	struct idtp9220_device_info *di = container_of(
		work, struct idtp9220_device_info, hall4_irq_work.work);
	...
	if (di->is_reverse_mode || di->is_boost_mode) {
		...
		if (int_val & INT_GET_CFG) {
			...
			if (di->is_reverse_mode || di->is_boost_mode) {
				di->is_reverse_chg = 4;
				schedule_delayed_work(
					&di->reverse_sent_state_work, 100);
			}
		}
...
static irqreturn_t idtp9220_hall3_irq_handler(int irq, void *dev_id)
{
	...
	if (gpio_is_valid(di->dt_props.hall3_gpio)) {
		if (gpio_get_value(di->dt_props.hall3_gpio)) {
			dev_info(di->dev,
				 "idtp9220_hall3_irq_handler: pen detach\n");
			di->hall3_online = 0;
			if (di->hall4_online) {
				dev_info(
					di->dev,
					"idtp9220_hall3_irq_handler: hall4 online, return\n");
				return IRQ_HANDLED;
			}
			schedule_delayed_work(&di->hall3_irq_work,
					      msecs_to_jiffies(0));
			return IRQ_HANDLED;
		} else {
			dev_info(di->dev,
				 "idtp9220_hall3_irq_handler: pen attach\n");
			di->hall3_online = 1;
		}
	}
...
static irqreturn_t idtp9220_hall4_irq_handler(int irq, void *dev_id)
{
	...
	if (gpio_is_valid(di->dt_props.hall4_gpio)) {
		if (gpio_get_value(di->dt_props.hall4_gpio)) {
			dev_info(di->dev,
				 "idtp9220_hall4_irq_handler: pen detach\n");
			di->hall4_online = 0;
			if (di->hall3_online) {
				dev_info(
					di->dev,
					"idtp9220_hall4_irq_handler: hall3 online, return\n");
				return IRQ_HANDLED;
			}
			schedule_delayed_work(&di->hall4_irq_work,
					      msecs_to_jiffies(0));
			return IRQ_HANDLED;
		} else {
			dev_info(di->dev,
				 "idtp9220_hall4_irq_handler: pen attach\n");
			di->hall4_online = 1;
		}
	}
...
{
	struct idtp9220_device_info *di = container_of(
		work, struct idtp9220_device_info, reverse_chg_work.work);
...
case POWER_SUPPLY_PROP_REVERSE_CHG_MODE:
		...
		if (di->hall3_online || di->hall4_online) {
			if (di->is_reverse_mode || di->is_boost_mode) {
				dev_info(di->dev,
					 "reverse charge running, return\n");
				break;
			}
		} else {
			dev_info(di->dev,
				 "pen detach, don't open reverse charge\n");
			break;
		}
