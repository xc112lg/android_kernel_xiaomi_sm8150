...
static int idtp9220_get_vrect(struct idtp9220_device_info *di)
{
	...
	di->vrect =
		di->vrect * 10 * 21 * 1000 / 40950; //vrect = val/4095*10*2.1
...
{
	struct idtp9220_device_info *di = container_of(
		work, struct idtp9220_device_info, mophie_tx_work.work);
	int ret = 0;
	u8 buf[2] = { 0 };
	u16 int_val = 0;
...
POWER_SUPPLY_PROP_CAPACITY, &val);
	...
	if (di->epp && (vinc || di->is_epp_qc3 || di->is_vin_limit) &&
	    soc < 95) {
		ret = idtp9220_get_property_names(di);
		if (di->wireless_psy) {
			val.intval = 1;
...
di->power_good_flag) {
	...
	if (adapter_vol > 0 && adapter_vol != di->last_vin) {
		...
		if (((di->tx_charger_type == ADAPTER_QC2) ||
			...
			dev_info(
				di->dev,
				"don't set curr with qc2/qc3 of bpp when vout low\n");
		else {
	...
	printk("[idtp] di->status:0x%x,adapter_vol =%d,icl_curr=%d, vbuck=%d, last_vin=%d, last_icl=%d\n",
	       di->status, adapter_vol, icl_curr, di->vbuck, di->last_vin,
	       di->last_icl);
...
union power_supply_propval val = {
	...
	if (gpio_is_valid(di->dt_props.wpc_det_gpio)) {
		...
		if (ret) {
			...
			schedule_delayed_work(&di->mophie_tx_work,
...
*/
	...
	if (int_val & INT_TX_DATA_RECV) {
		...
		case BC_TX_HWID:
			if (recive_data[4] == 0x01 && recive_data[2] == 0x2 &&
			    recive_data[3] == 0x8 && recive_data[1] == 0x6) {
				di->is_car_tx = 1;
			}
			idtp922x_request_adapter(di);
			break;
